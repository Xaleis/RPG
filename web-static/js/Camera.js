var Camera = function(player){
	...
	this.player.addPositionListener(function(x, y){self.refreshView(x, y);});
...
Camera.prototype.shake = function(factor){
	...
	var from = 0;
	var to = 60;
	$.ease(from, to, function(o){
		self.setViewPosition(self.legacyX + Math.round((Math.random()*2-1)*factor*Camera.SHAKE_SCREEN_DISTANCE), self.legacyY + Math.round((Math.random()*2-1)*factor*Camera.SHAKE_SCREEN_DISTANCE));
	},  {
		duration: Math.round(Camera.SHAKE_SCREEN_DURATION * factor),
		easing: "easeOutExpo",
		complete: function(){self.setViewPosition(self.legacyX, self.legacyY);}
	});
};
